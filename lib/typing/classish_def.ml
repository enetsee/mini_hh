...
let this_bounds classish_def =
  ...
  let Lang.Classish_def.
        { name; kind; require_extends; require_implements; require_class; _ }
    =
    classish_def
  in
  match kind.elem with
  | Lang.Classish_def.Kind.Class { is_final; _ } ->
    (match is_final with
     | Some final_span ->
     ...
     | _ ->
       ...
       Ty.Param_bounds.create ~upper:nom_upper ~lower ())
  | Lang.Classish_def.Kind.Intf ->
    let req_uppers = List.map require_extends ~f:ty_ctor_located in
    let prov =
      Prov.witnesses
      @@ (Located.span_of name
          :: List.map require_extends ~f:(fun Located.{ span; _ } -> span))
    in
  ...
  | Lang.Classish_def.Kind.Trait ->
    let req_uppers =
      List.map (require_extends @ require_implements) ~f:ty_ctor_located
    in
  ...
  let cont_ctxt =
    let ty_param =
      let this_ty_param =
        ...
        let name =
          Located.create
            ~elem:Name.Ty_param.this
            ~span:(Located.span_of name)
            ()
        in

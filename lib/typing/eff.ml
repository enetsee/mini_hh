...
type _ Effect.t +=
  ...
  | Log_enter_synth_expr : enter_synth_expr -> (Lang.Expr.t * Ctxt.Def.t * Ctxt.Cont.t) Effect.t
  | Log_enter_check_expr : enter_check_expr -> (Lang.Expr.t * Ty.t * Ctxt.Def.t * Ctxt.Cont.t) Effect.t
  | Log_exit_expr : exit_expr -> (Ty.t * Ctxt.Cont.Expr_delta.t) Effect.t
  | Log_enter_stmt : enter_stmt -> (Lang.Stmt.t * Ctxt.Def.t * Ctxt.Cont.t) Effect.t
  | Log_exit_stmt : exit_stmt -> Ctxt.Delta.t Effect.t
  | Log_enter_classish_def : enter_classish_def -> (Lang.Classish_def.t Located.t * Ctxt.Def.t * Ctxt.Cont.t) Effect.t
  | Log_exit_classish_def : Span.t -> unit Effect.t
  | Log_enter_fn_def : enter_fn_def -> (Lang.Fn_def.t Located.t * Ctxt.Def.t * Ctxt.Cont.t) Effect.t
...
let log_enter_synth_expr expr ctxt_def ctxt_cont = Effect.perform (Log_enter_synth_expr { expr; ctxt_def; ctxt_cont })
...
let log_enter_stmt stmt ctxt_def ctxt_cont = Effect.perform (Log_enter_stmt { stmt; ctxt_def; ctxt_cont })
...
let log_enter_fn_def fn_def ctxt_def ctxt_cont = Effect.perform (Log_enter_fn_def { fn_def; ctxt_def; ctxt_cont })
    ...
    { effc =
        (fun (type a) (eff : a Effect.t) ->
          ...
          | Log_enter_synth_expr { expr; ctxt_def; ctxt_cont } ->
            Some (fun (k : (a, _) Effect.Deep.continuation) -> Effect.Deep.continue k (expr, ctxt_def, ctxt_cont))
          | Log_enter_check_expr { expr; against; ctxt_def; ctxt_cont } ->
            Some
              (fun (k : (a, _) Effect.Deep.continuation) -> Effect.Deep.continue k (expr, against, ctxt_def, ctxt_cont))
          | Log_enter_stmt { stmt; ctxt_def; ctxt_cont } ->
            Some (fun (k : (a, _) Effect.Deep.continuation) -> Effect.Deep.continue k (stmt, ctxt_def, ctxt_cont))
          | Log_enter_classish_def { classish_def; ctxt_def; ctxt_cont } ->
            Some
              (fun (k : (a, _) Effect.Deep.continuation) -> Effect.Deep.continue k (classish_def, ctxt_def, ctxt_cont))
          | Log_enter_fn_def { fn_def; ctxt_def; ctxt_cont } ->
            Some (fun (k : (a, _) Effect.Deep.continuation) -> Effect.Deep.continue k (fn_def, ctxt_def, ctxt_cont))

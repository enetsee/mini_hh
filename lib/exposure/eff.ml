...
type _ Effect.t +=
  ...
  | Ask_ctor :
      Name.Ctor.t
      -> (Lang.Ty_param_def.t list * Ty.t list Name.Ctor.Map.t) option Effect.t
  ...
  | Log_enter_union :
      (Prov.t * Ty.t list) enter
      -> (Prov.t * Ty.t list * Ctxt.Ty_param.t * Dir.t) Effect.t
  | Log_enter_inter :
      (Prov.t * Ty.t list) enter
      -> (Prov.t * Ty.t list * Ctxt.Ty_param.t * Dir.t) Effect.t
  | Log_enter_fn :
      (Prov.t * Ty.Fn.t) enter
      -> (Prov.t * Ty.Fn.t * Ctxt.Ty_param.t * Dir.t) Effect.t
  | Log_enter_tuple :
      (Prov.t * Ty.Tuple.t) enter
      -> (Prov.t * Ty.Tuple.t * Ctxt.Ty_param.t * Dir.t) Effect.t
  | Log_enter_ctor :
      (Prov.t * Ty.Ctor.t) enter
      -> (Prov.t * Ty.Ctor.t * Ctxt.Ty_param.t * Dir.t) Effect.t
  | Log_enter_exists :
      (Prov.t * Ty.Exists.t) enter
      -> (Prov.t * Ty.Exists.t * Ctxt.Ty_param.t * Dir.t) Effect.t
...
let log_enter_cont_delta cont_delta =
  Effect.perform (Log_enter_cont_delta cont_delta)
;;

let log_enter_ty ty ty_params dir =
  Effect.perform (Log_enter_ty { elem = ty; ty_params; dir })
;;

let log_exit_cont_delta cont_delta =
  Effect.perform (Log_exit_cont_delta cont_delta)
;;

let log_enter_generic prov name ty_params dir =
  Effect.perform (Log_enter_generic { elem = prov, name; ty_params; dir })
;;

let log_enter_union prov tys ty_params dir =
  Effect.perform (Log_enter_union { elem = prov, tys; ty_params; dir })
;;

let log_enter_inter prov tys ty_params dir =
  Effect.perform (Log_enter_inter { elem = prov, tys; ty_params; dir })
;;

let log_enter_fn prov fn ty_params dir =
  Effect.perform (Log_enter_fn { elem = prov, fn; ty_params; dir })
;;

let log_enter_tuple prov tuple ty_params dir =
  Effect.perform (Log_enter_tuple { elem = prov, tuple; ty_params; dir })
;;

let log_enter_ctor prov ctor ty_params dir =
  Effect.perform (Log_enter_ctor { elem = prov, ctor; ty_params; dir })
;;
    ...
    { effc =
        (fun (type a) (eff : a Effect.t) ->
          ...
          | Ask_ctor nm ->
            ...
            Some
              (fun (k : (a, _) Effect.Deep.continuation) ->
                Effect.Deep.continue k ty_params_supers)
          | Log_enter_delta delta ->
            Some
              (fun (k : (a, _) Effect.Deep.continuation) ->
                Effect.Deep.continue k delta)
          | Log_exit_delta delta ->
            Some
              (fun (k : (a, _) Effect.Deep.continuation) ->
                Effect.Deep.continue k delta)
          ...
          | Log_enter_ty { elem = ty; ty_params; dir } ->
            Some
              (fun (k : (a, _) Effect.Deep.continuation) ->
                Effect.Deep.continue k (ty, ty_params, dir))
          | Log_enter_generic { elem = prov, name; ty_params; dir } ->
            Some
              (fun (k : (a, _) Effect.Deep.continuation) ->
                Effect.Deep.continue k (prov, name, ty_params, dir))

...
let rec step_tuple_elem prov_sub params_sub prov_super params_super ~idx_sub ~idx_super ~cstrs =
  (* TODO(mjt) PROV!!!! *)
  ...
  | (ty_sub :: reqs_sub, opts_sub, var_sub), (ty_super :: reqs_sup, opts_sup, var_sup) ->
    let cstr = Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super in
  ...
  | ([], ty_sub :: opts_sub, var_sub), ([], ty_super :: opts_sup, var_sup) ->
    let cstr = Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super in
    let cstrs = cstr :: cstrs in
    step_tuple_elem prov_sub ([], opts_sub, var_sub) prov_super ([], opts_sup, var_sup) ~idx_sub ~idx_super ~cstrs
  ...
  | ([], ty_sub :: opts_sub, var_sub), (ty_super :: reqs_sup, opts_sup, var_sup) ->
    ...
    let cstr = Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super in
    let cstrs = cstr :: cstrs in
    step_tuple_elem prov_sub ([], opts_sub, var_sub) prov_super (reqs_sup, opts_sup, var_sup) ~idx_sub ~idx_super ~cstrs
  ...
  | ([], [], Some ty_sub), (ty_super :: reqs_sup, opts_sup, var_sup) ->
    ...
    let cstr = Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super in
    let cstrs = cstr :: cstrs in
    step_tuple_elem prov_sub ([], [], Some ty_sub) prov_super (reqs_sup, opts_sup, var_sup) ~idx_sub ~idx_super ~cstrs
  ...
  | ([], [], Some ty_sub), ([], ty_super :: opts_sup, var_sup) ->
    let cstr = Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super in
    let cstrs = cstr :: cstrs in
    step_tuple_elem prov_sub ([], [], Some ty_sub) prov_super ([], opts_sup, var_sup) ~idx_sub ~idx_super ~cstrs
  (* -- Match a variadic param in the supertype with an optional param in the subtype -- *)
  | ([], ty_sub :: opts_sub, var_sub), ([], [], Some ty_super) ->
    let cstr = Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super in
    let cstrs = cstr :: cstrs in
    step_tuple_elem prov_sub ([], opts_sub, var_sub) prov_super ([], [], Some ty_super) ~idx_sub ~idx_super ~cstrs
  (* ~~ Error conditions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  (* -- Fewer required params in supertype than in subtype -- *)
  | ((_ :: _ as reqs), _, _), ([], _, _) ->
    let n_sub = idx_sub + List.length reqs in
    Error (Err.tuple_arity_required ~prov_sub ~prov_super ~n_sub ~n_super:idx_super)
  (* -- Required params in supertype but no params in subtype -- *)
  | ([], [], None), ((_ :: _ as reqs), _, _) ->
    let n_super = idx_super + List.length reqs in
    Error (Err.tuple_arity_required ~prov_sub ~prov_super ~n_sub:idx_sub ~n_super)
  (* -- More optional params in the supertype with no params in the subtype -- *)
  | ([], [], None), ([], (_ :: _ as opts), _) ->
    let n_super = idx_super + List.length opts in
    Error (Err.tuple_arity_optional ~prov_sub ~prov_super ~n_sub:idx_sub ~n_super)
  (* -- Variadic param in the supertype with no params in the subtype -- *)
  | ([], [], None), ([], [], Some _) -> Error (Err.tuple_arity_variadic ~prov_sub ~prov_super)
;;

let step_tuple
  ~prov_sub
  ~tuple_sub:Ty.Tuple.{ required = reqs_sub; optional = opts_sub; variadic = var_sub }
  ~prov_super
  ~tuple_super:Ty.Tuple.{ required = reqs_super; optional = opts_super; variadic = var_super }
  =
  ...
  @@ step_tuple_elem prov_sub params_sub prov_super params_super ~idx_sub:0 ~idx_super:0 ~cstrs:[]
;;
  ...
  let rec aux ~idx ~args_sub ~args_super ~vs ~props =
    ...
    | arg_sub :: args_sub, arg_super :: args_super, variance :: vs ->
      ...
      let props =
        ...
        | Variance.Cov ->
          let prop = Prop.atom @@ Cstr.is_subtype ~ty_sub:arg_sub ~ty_super:arg_super in
          prop :: props
        | Variance.Contrav ->
          let prop = Prop.atom @@ Cstr.is_subtype ~ty_sub:arg_super ~ty_super:arg_sub in
          prop :: props
        | Variance.Inv ->
          let prop_cov = Prop.atom @@ Cstr.is_subtype ~ty_sub:arg_sub ~ty_super:arg_super
          and prop_cotrav = Prop.atom @@ Cstr.is_subtype ~ty_sub:arg_super ~ty_super:arg_sub in
...
let step ~ty_sub ~ty_super ~ctxt_cont =
  ...
  (* ~~ C-Top ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (_prov_sub, (Union _ | Inter _ | Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)), (_prov_super, Inter [])
    -> Ok Prop.true_
  | ( (_prov_sub, Inter [])
    , (_prov_super, (Union _ | Inter (_ :: _) | Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)) ) ->
    Error (Err.not_a_subtype ~ty_sub ~ty_super)
  (* ~~ C-Bot ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (_, Union []), (_, (Union _ | Inter _ | Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)) -> Ok Prop.true_
  | (_, (Union (_ :: _) | Inter _ | Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)), (_, Union []) ->
    Error (Err.not_a_subtype ~ty_sub ~ty_super)
  (* ~~ C-Union-L ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | ( (prov_sub, Union tys_sub)
    , (_, (Union (_ :: _) | Inter (_ :: _) | Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)) ) ->
    let props =
    ...
    Ok (Prop.conj props)
  (* ~~ C-Inter-R ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (_, (Inter (_ :: _) | Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)), (prov_super, Inter tys_super) ->
    let props =
    ...
    Ok (Prop.conj props)
  (* ~~ C-Union-R ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (_, (Inter (_ :: _) | Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)), (prov_super, Union tys_super) ->
    let props =
      List.map tys_super ~f:(fun ty ->
    ...
    Ok (Prop.disj props)
  (* ~~ C-Inter-L ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (prov_sub, Inter tys_sub), (_, (Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)) ->
    let props =
      List.map tys_sub ~f:(fun ty ->
  ...
  (* ~~ C-Var ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  (* ~~ C-Generic ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (_prov_sub, Generic name_sub), (_prov_super, Generic name_super) when Name.Ty_param.equal name_sub name_super ->
    Ok Prop.true_
  | (_, (Exists _ | Fn _ | Generic _ | Tuple _ | Ctor _ | Base _)), (prov_super, Generic name_super) ->
    let ty_super =
      let Ty.Param_bounds.{ lower; _ } = Option.value_exn @@ Ctxt.Cont.ty_param_bounds ctxt_cont name_super in
      Ty.map_prov lower ~f:(fun bound -> Prov.axiom_lower_bound ~bound ~of_:prov_super)
    in
    Ok (Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super)
  | (prov_sub, Generic name_sub), (_prov_super, (Exists _ | Fn _ | Tuple _ | Ctor _ | Base _)) ->
    let ty_sub =
      let Ty.Param_bounds.{ upper; _ } = Option.value_exn @@ Ctxt.Cont.ty_param_bounds ctxt_cont name_sub in
      Ty.map_prov upper ~f:(fun bound -> Prov.axiom_upper_bound ~bound ~of_:prov_sub)
    in
    Ok (Prop.atom @@ Cstr.is_subtype ~ty_sub ~ty_super)
  (* ~~ C-Exists ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (_prov_sub, Exists _exists_sub), (_prov_super, Exists _exists_super) -> Error (Err.not_a_subtype ~ty_sub ~ty_super)
  | (_prov_sub, (Fn _ | Tuple _ | Ctor _ | Base _)), (_prov_super, Exists _exist_super) ->
    Error (Err.not_a_subtype ~ty_sub ~ty_super)
  | (_prov_sub, Exists _exists_sub), (_prov_super, (Fn _ | Tuple _ | Ctor _ | Base _)) ->
    Error (Err.not_a_subtype ~ty_sub ~ty_super)
  (* ~~ C-Fn ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (prov_sub, Fn fn_sub), (prov_super, Fn fn_super) -> step_fn ~prov_sub ~fn_sub ~prov_super ~fn_super
  | (_prov_sub, (Tuple _ | Ctor _ | Base _)), (_prov_super, Fn _fn_super) -> Error (Err.not_a_subtype ~ty_sub ~ty_super)
  | (_prov_sub, Fn _fn_sub), (_prov_super, (Tuple _ | Ctor _ | Base _)) -> Error (Err.not_a_subtype ~ty_sub ~ty_super)
  (* ~~ C-Ctor ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (prov_sub, Ctor ctor_sub), (prov_super, Ctor ctor_super) -> step_ctor ~prov_sub ~ctor_sub ~prov_super ~ctor_super
  | (_prov_sub, Ctor _ctor_sub), (_prov_super, (Tuple _ | Base _)) -> Error (Err.not_a_subtype ~ty_sub ~ty_super)
  | (_prov_sub, (Tuple _ | Base _)), (_prov_super, Ctor _ctor_super) -> Error (Err.not_a_subtype ~ty_sub ~ty_super)
  (* ~~ C-Tuple ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  | (prov_sub, Tuple tuple_sub), (prov_super, Tuple tuple_super) ->
    step_tuple ~prov_sub ~tuple_sub ~prov_super ~tuple_super
  | (_prov_sub, Base _), (_prov_super, Tuple _tuple_super) -> Error (Err.not_a_subtype ~ty_sub ~ty_super)
  | (_prov_sub, Tuple _tuple_sub), (_prov_super, Base _) -> Error (Err.not_a_subtype ~ty_sub ~ty_super)
  (* ~~ C-Base ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)

...
let elab_to_generic { name; lambda; where_constraints } ~bound_ty_params =
  ...
  let Expr_stmt.Lambda_sig.{ ty_params; params; return } = Located.elem lambda_sig in
  ...
  let lambda =
    let lambda_sig =
      ...
      and params = Expr_stmt.Fn_param_defs.elab_to_generic params ~bound_ty_params
  ...
  in
  let where_constraints = List.map ~f:(Ty.Param.elab_to_generic ~bound_ty_params) where_constraints in

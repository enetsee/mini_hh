...
(* ~~ Flow constructors ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
let refine ~prov_test ~prov_scrut = Flow (Refine { scrut = prov_scrut; test = prov_test })
let unpack ~prov_packed ~prov_unpacked = Flow (Unpack { packed = prov_packed; unpacked = prov_unpacked })
let assign ~prov_rhs ~prov_lvalue = Flow (Assign { rhs = prov_rhs; lvalue = prov_lvalue })
let use ~prov_def ~prov_tm_var = Flow (Use { def = prov_def; tm_var = prov_tm_var })
...
let prj_ctor_co ~sub ~sub_prj ~super kind name idx is_invariant =
  ...
  let cstr_variance = if is_invariant then Projection.Cstr_variance.Inv dir else Projection.Cstr_variance.Dir dir in
...
let axiom_extends ~child ~ancestor = Axiom { axiom = Axiom.Extends; prev = child; next = ancestor }
let axiom_upper_bound ~bound ~of_ = Axiom { axiom = Upper_bound; prev = of_; next = bound }
let axiom_lower_bound ~bound ~of_ = Axiom { axiom = Lower_bound; prev = of_; next = bound }
